package com.zhiyicx.zhibolibrary.ui.components.sweetsheet.adapter;
...
import com.zhiyicx.zhibolibrary.ui.components.sweetsheet.listener.SingleClickListener;
...
public class MenuRVAdapter extends RecyclerView.Adapter<MenuRVAdapter.MenuVH> {
    ...
    public MenuRVAdapter(List<MenuEntity> dataLis, SweetSheet.Type type) {
        ...
        if (type == SweetSheet.Type.RecyclerView) {
            mItemLayoutId = R.layout.zb_item_horizon_rv;
        }
        else {
            mItemLayoutId = R.layout.zb_item_gift_rv;
        }
    }
    ...
    public void onBindViewHolder(MenuVH menuVH, int i) {
        ...
        if (!TextUtils.isEmpty(menuEntity.iconId)) {
            menuVH.iv.setVisibility(View.VISIBLE);
            if (!TextUtils.isEmpty(DataHelper.getStringSF(menuEntity.iconId,UiUtils.getContext())))
                try {
                    menuVH.iv.setImageBitmap(UiUtils.getLoacalBitmap(menuEntity.iconId));
                } catch (Exception e) {
                    e.printStackTrace();
                    UiUtils.glideWrap(menuEntity.iconId).into(menuVH.iv);
                }
            else
                UiUtils.glideWrap(menuEntity.iconId).into(menuVH.iv);
        ...
        menuVH.num.setText(" x " + menuEntity.gold);
        if (menuEntity.isChecked)
            menuVH.iv.setBackgroundResource(R.drawable.selector_blue_box);
        else
            menuVH.iv.setBackground(null);
    ...
    public void refresh(List<MenuEntity> mDataList) {
