...
//                            mRootView.hideRefreshing();
    ...
    private void filterRefresh(final boolean isMore) {
        ...
        if (usids.length() > 0) {
            mModel.getUsidInfo(usids, "").observeOn(AndroidSchedulers.mainThread()).subscribe(new Action1<BaseJson<UserInfo[]>>() {
                @Override
    ...
    private void notFilterWrapRefresh(final boolean isMore) {
        ...
        if (usids.length() > 0) {
            ...
            mModel.getUsidInfo(usids, "").subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new Action1<BaseJson<UserInfo[]>>() {
                @Override
                public void call(BaseJson<UserInfo[]> baseJson) {
                    if (baseJson.code.equals(ZBLApi.REQUEST_SUCESS)) {
                        UserInfo[] userInfos = baseJson.data;
