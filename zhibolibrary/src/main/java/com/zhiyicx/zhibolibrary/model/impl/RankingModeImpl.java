...
public class RankingModeImpl implements RankingModel {
    ...
    public RankingModeImpl(ServiceManager manager) {
        this.mUserService = manager.getUserService();
    }

    @Override
    public Observable<BaseJson<SearchResult[]>> getRanking(int page) {
        Map<String, Object> map = new HashMap<>();
        map.put("p", page);

        return ZBCloudApiClient.getInstance().sendCloudApiRequestForRx(ZBLApi.ZB_API_GET_PRESENTER_RANK_LIST, map).observeOn(Schedulers.io())
                .map(new Func1<JsonObject, BaseJson<SearchResult[]>>() {
                    @Override
