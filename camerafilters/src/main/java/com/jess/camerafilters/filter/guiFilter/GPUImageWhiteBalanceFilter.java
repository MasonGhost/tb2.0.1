...
public class GPUImageWhiteBalanceFilter extends GPUImageFilter {
    ...
    private float mTemperature = 5000;
    private int mTintLocation;
    private float mTint = 0;
    ...
    public int createProgram(Context applicationContext) {
        return GlUtil.createProgram(NO_FILTER_VERTEX_SHADER,WHITE_BALANCE_FRAGMENT_SHADER);
    }

    @Override
    public void getGLSLValues() {
        super.getGLSLValues();
        mTemperatureLocation = GLES20.glGetUniformLocation(mProgramHandle, "temperature");
        mTintLocation = GLES20.glGetUniformLocation(mProgramHandle, "tint");
    }
    ...
    protected void bindGLSLValues(float[] mvpMatrix, FloatBuffer vertexBuffer, int coordsPerVertex, int vertexStride, float[] texMatrix, FloatBuffer texBuffer, int texStride) {
        ...
        GLES20.glUniform1f(mTemperatureLocation, mTemperature < 5000 ? (float) (0.0004 * (mTemperature - 5000.0)) : (float) (0.00006 * (mTemperature - 5000.0)));
        GLES20.glUniform1f(mTintLocation, (float) (mTint/100.0));
    }
