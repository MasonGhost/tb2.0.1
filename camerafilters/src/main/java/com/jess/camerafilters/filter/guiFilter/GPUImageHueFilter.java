...
public class GPUImageHueFilter extends GPUImageFilter {
    ...
    private float mHue = 90.0f;
    private int mHueLocation;

    public GPUImageHueFilter(Context applicationContext) {
    ...
    public int createProgram(Context applicationContext) {
        return GlUtil.createProgram(NO_FILTER_VERTEX_SHADER,HUE_FRAGMENT_SHADER);
    }

    @Override
    public void getGLSLValues() {
        super.getGLSLValues();
        mHueLocation = GLES20.glGetUniformLocation(mProgramHandle, "hueAdjust");
    }
    ...
    protected void bindGLSLValues(float[] mvpMatrix, FloatBuffer vertexBuffer, int coordsPerVertex, int vertexStride, float[] texMatrix, FloatBuffer texBuffer, int texStride) {
        ...
        GLES20.glUniform1f(mHueLocation, hueAdjust);
    }
