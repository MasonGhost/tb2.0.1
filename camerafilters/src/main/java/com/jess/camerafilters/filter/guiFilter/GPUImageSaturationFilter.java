...
public class GPUImageSaturationFilter extends GPUImageFilter {
    ...
    public GPUImageSaturationFilter(Context context) {
        super( context);
        mSaturation = 0.0f;//s 0.0f-1.0f
    }

    @Override
    public int createProgram(Context applicationContext) {
        return GlUtil.createProgram(NO_FILTER_VERTEX_SHADER,SATURATION_FRAGMENT_SHADER);
    }

    @Override
    public void getGLSLValues() {
        super.getGLSLValues();
        mSaturationLocation = GLES20.glGetUniformLocation(mProgramHandle, "saturation");
    }
