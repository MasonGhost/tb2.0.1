...
public class RegisterActivityTest {
    ...
    public void inputUsernameCheck() throws Exception {
        onView(withId(R.id.et_regist_username)).perform(typeText(USER_NAME), closeSoftKeyboard());
     ...
     * 2. 填写正确手机号、验证码、密码
     * 3. 点击注册
     * expected                     字符为空时，不能点击“注册”；用户名过短，提示应为3-8个字符；
     * 用户名达到8位后，不能再输入
     *
     * @throws Exception
     */
    ...
    public void usernameLengthCheck() throws Exception {
        onView(withId(R.id.et_regist_username)).perform(typeText(USER_NAME));
