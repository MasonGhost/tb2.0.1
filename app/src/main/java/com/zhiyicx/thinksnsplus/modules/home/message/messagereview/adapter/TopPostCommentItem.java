...
public class TopPostCommentItem extends BaseTopItem implements BaseTopItem.TopReviewEvetnInterface{
    ...
    public void convert(ViewHolder holder, BaseListBean baseListBean, BaseListBean lastT, int position, int itemCounts) {
        ...
        ImageUtils.loadCircleUserHeadPic(postCommentListBean.getCommentUser(), holder.getView(R.id.iv_headpic));

        TextView review_flag = holder.getTextView(R.id.tv_review);
        if (postCommentListBean.getState() == TopNewsCommentListBean.TOP_REVIEW) {
        ...
        if (postCommentListBean.getPost() != null && postCommentListBean.getPost().getImages() != null
                &&!postCommentListBean.getPost().getImages().isEmpty()) {
            holder.setVisible(R.id.iv_detail_image, View.VISIBLE);
            Glide.with(holder.itemView.getContext())
                    .load(ImageUtils.imagePathConvertV2(postCommentListBean.getPost().getImages().get(0).getFile_id()
        ...
        if (postCommentListBean.getPost() == null || postCommentListBean.getComment() == null) {
            ...
            review_flag.setTextColor(holder.itemView.getResources().getColor(R.color.message_badge_bg));
            review_flag.setText(holder.itemView.getResources().getString(postCommentListBean.getPost() == null ?
        ...
        holder.setText(R.id.tv_time, TimeUtils.getTimeFriendlyNormal(postCommentListBean.getUpdated_at()));


        // 响应事件
        RxView.clicks(holder.getView(R.id.tv_name))
                .throttleFirst(JITTER_SPACING_TIME, TimeUnit.SECONDS)
                .subscribe(aVoid -> toUserCenter(holder.itemView.getContext(), postCommentListBean.getCommentUser()));
        ...
        RxView.clicks(holder.itemView)
                ...
                .subscribe(aVoid -> {
                    ...
                    toDetail(postCommentListBean.getComment());
                });
    ...
    public void onReviewApprovedClick(BaseListBean data, int position) {
        ...
        postCommentListBean.setExpires_at(TimeUtils.millis2String(System.currentTimeMillis() + 1000000));
