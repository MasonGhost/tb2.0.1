...
//            mScaleCenter.set(detector.getFocusX(), detector.getFocusY());
     ...
     */
    public void animaFrom(ImageInfo ImageInfo) {
        if (isInit) {
            reset();
            ...
            float ocx = ImageInfo.mRect.left + ImageInfo.mRect.width() / 2;
            float ocy = ImageInfo.mRect.top + ImageInfo.mRect.height() / 2;
    ...
    public void animaTo(ImageInfo ImageInfo, Runnable completeCallBack) {
        if (isInit) {
            mTranslate.stop();
            ...
            mTranslate.withRotate((int) mDegrees, (int) ImageInfo.mDegrees, mAnimaDuring * 2 / 3);
