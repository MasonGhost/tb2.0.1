...
public class HttpUrlSource implements Source {
    ...
    public void open(int offset) throws ProxyCacheException {
        try {
            ...
            throw new ProxyCacheException("Error opening connection for " + sourceInfo.url + " " +
                    "with offset " + offset, e);
        }
    }

    private int readSourceAvailableBytes(HttpURLConnection connection, int offset, int
        ...
        if (connection != null) {
            try {
                ...
                String message = "Wait... but why? WTF!? " +
                        ...
                        "or create issue here https://github" +
                        ".com/danikula/AndroidVideoCache/issues.";
    ...
    private HttpURLConnection openConnection(int offset, int timeout) throws IOException,
            ProxyCacheException {
        ...
        do {
            ...
            redirected = code == HTTP_MOVED_PERM || code == HTTP_MOVED_TEMP || code ==
                    HTTP_SEE_OTHER;
            if (redirected) {
    ...
    private HttpURLConnection openConnectionForHeader(int offset, int timeout) throws
            IOException, ProxyCacheException {
        HttpURLConnection connection;
        boolean redirected;
        int redirectCount = 0;
        String url = this.sourceInfo.url;
        do {
            LOG.error("openConnectionForHeader", "Open connection for header to " + url);
            connection = (HttpURLConnection) new URL(url).openConnection();
            if (offset > 0) {
                connection.setRequestProperty("Range", "bytes=" + offset + "-");
            }
            ...
            if (redirected) {
                url = connection.getHeaderField("Location");
                LOG.error("openConnectionForHeader", "Redirect to:" + url);
                redirectCount++;
                connection.disconnect();
                LOG.error("openConnectionForHeader", "Redirect closed:" + url);
            }
            if (redirectCount > MAX_REDIRECTS) {
                throw new ProxyCacheException("Too many redirects: " + redirectCount);
            }
